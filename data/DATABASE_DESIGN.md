...
| Sheet | 行数 | 说明 |
|-------|------|------|
| **Data** | 269 | 主数据：各国 GDP 按年份展开 |
| **Metadata - Countries** | 265 | 国家元数据（区域、收入分组等） |
...
- **国家数量**：约 265 条（含部分区域合计行，如 AFE、AFW）
...
|------|------|------|------|
| id | BIGINT | PK, AUTO_INCREMENT | 主键 |
| country_id | BIGINT | NOT NULL, FK → wb_country | 国家 |
...
**唯一约束**：`(country_id, indicator_id, gdp_year)` — 一国一年一指标仅一条记录。
...
世界银行.xls
    │
    ├─ Metadata - Countries  ──> wb_country
    ├─ Metadata - Indicators ──> wb_indicator
    └─ Data (宽表)           ──> wb_gdp_value (窄表：一国一年一行)
```
...
| dataset[].Country | `country_name`（可转大写） |
| dataset[].Year | `year` |
| countryCodes | `country_code` 映射为 ISO2（需额外映射表或配置） |
| countryFlagImages | `/flags/{iso2}.png` |
