#!/usr/bin/env python3
"""从 MySQL 生成南亚 8 国 GDP 图表 JSON（1960 年起）。"""
...
OUTPUT_PATH = ROOT / "data" / "south-asia-gdp.json"
PUBLIC_OUTPUT_PATH = ROOT / "frontend" / "public" / "data" / "south-asia-gdp.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate south-asia-gdp.json from MySQL")
...
def fetch_rows(cursor, start_year: int):
    placeholders = ", ".join(["%s"] * len(SOUTH_ASIA_COUNTRY_CODES))
    sql = f"""
        SELECT
            c.country_code,
            c.country_name,
    ...
    """
    params = (INDICATOR_CODE, start_year, *SOUTH_ASIA_COUNTRY_CODES)
...
def build_chart(rows: list[tuple], start_year: int) -> dict:
    ...
    if not by_country:
        raise ValueError("No South Asia GDP data found in database")
    ...
    selected = [code for code in SOUTH_ASIA_COUNTRY_CODES if code in by_country]
    ...
    return {
        ...
        "_meta": {
            ...
            "note": "South Asia 8 entities (IND, PAK, BGD, LKA, NPL, BTN, MDV, AFG)",
        },
    }
