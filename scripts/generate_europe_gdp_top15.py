#!/usr/bin/env python3
"""从 MySQL 生成 Europe GDP TOP 15 图表 JSON（1960 年起，地理欧洲口径）。"""
...
from europe_countries import EUROPE_GDP_CODES  # noqa: E402
...
OUTPUT_PATH = ROOT / "data" / "europe-gdp-top15.json"
PUBLIC_OUTPUT_PATH = ROOT / "frontend" / "public" / "data" / "europe-gdp-top15.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate europe-gdp-top15.json from MySQL")
...
def fetch_rows(cursor, start_year: int):
    allowed = sorted(EUROPE_GDP_CODES)
    ...
    """
    # SUN 可能没有 income_group，单独再查一次并入
    ...
    sun_sql = """
        ...
        ORDER BY g.gdp_year
    """
    cursor.execute(sun_sql, (INDICATOR_CODE, start_year))
    sun_rows = cursor.fetchall()
    # 去重：若第一次已包含 SUN 则不会重复追加
    existing = {(r[0], r[2]) for r in rows}
    for r in sun_rows:
        if (r[0], r[2]) not in existing:
            rows.append(r)
    return rows
...
def build_chart(rows: list[tuple], start_year: int) -> dict:
    ...
    if not by_country:
        raise ValueError("No Europe GDP data found in database")

    has_sun = "SUN" in by_country
    all_years = sorted(
    ...
    for year in years:
        ranked = sorted(
            (
                ...
                and not (code == "RUS" and year <= 1991 and has_sun)
            ),
    ...
    for idx, code in enumerate(selected):
        ...
        for year in years:
            if code == "RUS" and year <= 1991 and has_sun:
                continue
