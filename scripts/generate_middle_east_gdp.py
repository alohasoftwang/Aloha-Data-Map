#!/usr/bin/env python3
"""从 MySQL 生成中东 GDP 图表 JSON（1960 年起）。"""
...
OUTPUT_PATH = ROOT / "data" / "middle-east-gdp.json"
PUBLIC_OUTPUT_PATH = ROOT / "frontend" / "public" / "data" / "middle-east-gdp.json"

START_YEAR = 1960
MAX_BARS = 15

# 与后端 ChartJsonExportService.MIDDLE_EAST_CODES 一致
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate middle-east-gdp.json from MySQL")
...
def fetch_rows(cursor, start_year: int):
    placeholders = ", ".join(["%s"] * len(MIDDLE_EAST_COUNTRY_CODES))
    sql = f"""
        SELECT
            c.country_code,
            c.country_name,
    ...
    """
    params = (INDICATOR_CODE, start_year, *MIDDLE_EAST_COUNTRY_CODES)
...
def build_chart(rows: list[tuple], start_year: int) -> dict:
    ...
    if not by_country:
        raise ValueError("No Middle East GDP data found in database")
    ...
    selected = [code for code in MIDDLE_EAST_COUNTRY_CODES if code in by_country]
