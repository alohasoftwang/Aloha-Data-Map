#!/usr/bin/env python3
"""从 MySQL 生成 South America GDP TOP 15 图表 JSON（1960 年起，地理南美口径）。"""
...
from south_america_countries import SOUTH_AMERICA_GDP_CODES  # noqa: E402
...
OUTPUT_PATH = ROOT / "data" / "south-america-gdp.json"
PUBLIC_OUTPUT_PATH = ROOT / "frontend" / "public" / "data" / "south-america-gdp.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate south-america-gdp.json from MySQL")
...
def fetch_rows(cursor, start_year: int):
    allowed = sorted(SOUTH_AMERICA_GDP_CODES)
...
def build_chart(rows: list[tuple], start_year: int) -> dict:
    ...
    if not by_country:
        raise ValueError("No South America GDP data found in database")
