#!/usr/bin/env python3
"""将台湾 GDP 补充数据写入 MySQL（wb_country + wb_gdp_value）。"""
...
DATA_PATH = ROOT / "data" / "twn_gdp_usd.json"
INDICATOR_CODE = "NY.GDP.MKTP.CD"
COUNTRY_CODE = "TWN"


def parse_args():
    parser = argparse.ArgumentParser(description="Import Taiwan GDP into MySQL")
...
def main() -> None:
    ...
    try:
        ...
        conn.commit()
        print(f"Removed old TWN GDP rows: {deleted}")
        print(f"Inserted TWN GDP rows: {inserted}")
