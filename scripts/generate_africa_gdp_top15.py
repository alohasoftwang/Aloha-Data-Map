#!/usr/bin/env python3
"""从 MySQL 生成 Africa GDP TOP 15 图表 JSON（1960 年起，地理非洲口径）。"""
...
from africa_countries import AFRICA_GDP_CODES  # noqa: E402
...
OUTPUT_PATH = ROOT / "data" / "africa-gdp-top15.json"
PUBLIC_OUTPUT_PATH = ROOT / "frontend" / "public" / "data" / "africa-gdp-top15.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate africa-gdp-top15.json from MySQL")
...
def fetch_rows(cursor, start_year: int):
    allowed = sorted(AFRICA_GDP_CODES)
...
def build_chart(rows: list[tuple], start_year: int) -> dict:
    ...
    if not by_country:
        raise ValueError("No Africa GDP data found in database")
